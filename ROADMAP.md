...
- [ ] **实验追踪 (MLOps Learning)**
    - [x] 定义指标协议：stdout 输出 `__METRIC__ : <JSON>` -> Kernel 发送 metric 消息。
    - [x] App Runs 面板：记录 run_id、状态、耗时、指标列表（本地持久化）。
    - [x] Lab 练习页：提供训练任务并用指标自动判定完成度。
- [ ] **基础安全与隔离**
    - [x] 设置代码执行超时时间 (Timeout)。
    - [x] 支持中断运行（取消当前执行任务）。
    - [x] 简单的 AST 检查（禁止 `os.system` 等高危操作 - 基础版）。
