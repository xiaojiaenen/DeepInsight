...
const AgentNode = ({ data }: { data: any }) => (
  <div className={`px-4 py-3 shadow-lg rounded-xl bg-white border-2 min-w-[150px] ${
...
export const AgentVisualizer: React.FC = () => {
  const nodes: Node[] = [
    { 
      id: 'input', 
      type: 'agent',
      data: { role: 'input', content: '用户：写一段贪吃蛇代码' }, 
      position: { x: 200, y: 0 } 
    },
    { 
      id: 'thought1', 
      type: 'agent',
      data: { role: 'thought', content: '需要使用 Python pygame 库。首先初始化游戏窗口。' }, 
      position: { x: 200, y: 100 } 
    },
    { 
      id: 'action1', 
      type: 'agent',
      data: { role: 'action', content: '调用搜索工具：pygame 基础模板' }, 
      position: { x: 50, y: 200 } 
    },
    { 
      id: 'obs1', 
      type: 'agent',
      data: { role: 'observation', content: '搜索结果：找到 pygame.init() 和循环模板' }, 
      position: { x: 50, y: 300 } 
    },
    { 
      id: 'thought2', 
      type: 'agent',
      data: { role: 'thought', content: '基于模板编写代码逻辑，包括蛇身移动和碰撞检测。' }, 
      position: { x: 200, y: 400 } 
    },
    { 
      id: 'output', 
      type: 'agent',
      data: { role: 'output', content: '已生成代码。点击下方查看。' }, 
